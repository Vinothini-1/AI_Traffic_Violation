...
export const loader = async ({ params }) => {
  try {
    const { data } = await customFetch(
      `/waste/retriveSpecificCollectedWaste/${params.id}`
    );
    return data;
  } catch (error) {
...
export const action = async ({ request, params }) => {
  ...
  try {
    const response = await customFetch.put(
      `/waste/updateCollectedWaste/${params.id}`,
      data
    );
        ...
        <Form method="post">
          ...
          <div className="mt-8">
            ...
            >
              ...
              <option value="Red Jump">Red Jump</option>
              <option value="Driving without licence">Driving without licence</option>
              <option value="RC violation">RC violation</option>
